...
async def start(client, message):
        ...
        for file in files:
            ...
            if CUSTOM_FILE_CAPTION:
                try:
                    ...
                    f_caption=f_caption
            if f_caption is None:
                f_caption = f"{' '.join(filter(lambda x: not x.startswith('@') and not x.startswith('_'), files1.file_name.split()))}"
    ...
    if not files_:
        ...
        try:
            ...
            title = '@TeamHMT ' + ' '.join(filter(lambda x: not x.startswith('@') and not x.startswith('_'), file.file_name.split()))
