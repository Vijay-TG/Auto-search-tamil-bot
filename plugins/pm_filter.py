...
from utils import get_size, filtered_file_name, is_subscribed, get_poster, search_gagala, temp, get_settings, save_group_settings, get_shortlink, get_tutorial, send_all
...
async def next_page(bot, query):
    ...
    text=f"[{get_size(file.file_size)}] {' '.join(filtered_file_name(file.file_id).split())}", callback_data=f'{pre}#{file.file_id}'
                ),
            ]
            for file in files
        ]
